...
export const load = (binary: string) => {
  ...
  let gpsIfd = null;
  let thumbnail = null;
  let pointer = _utils.unpack(exifReader.endianMark + 'L', exifReader.tiftag.slice(4, 8))[0];
  ...
  if (firstIfdPointer != '\x00\x00\x00\x00') {
    const pointer = _utils.unpack(exifReader.endianMark + 'L', firstIfdPointer)[0];
...
export const dump = (originalExifObj: interfaces.IExif) => {
  ...
  if (existFirstIfd) {
    ...
    const thumbnailPointerBinary = '\x02\x01\x00\x04\x00\x00\x00\x01' + _utils.pack('>L', [thumbnailPointer]);
    const thumbnailLengthBinary = '\x02\x02\x00\x04\x00\x00\x00\x01' + _utils.pack('>L', [thumbnail.length]);
    firstIfdBytes =
